...
export default async function handler(req,res){
 ...
 const {data:tokens,error:tErr}=await s.from("mcp_oauth_tokens").select("client_id,user_id,scope,expires_at,refresh_expires_at,created_at").is("revoked_at",null);
 ...
 const result=["chatgpt","claude","gemini"].map(key=>{const matches=(clients||[]).filter(c=>classify(c)===key);const ids=new Set(matches.map(c=>c.client_id));const now=Date.now();const active=(tokens||[]).filter(t=>ids.has(t.client_id));const authorized=active.filter(t=>new Date(t.refresh_expires_at||0).getTime()>now||new Date(t.expires_at||0).getTime()>now);const latest=authorized.slice().sort((a,b)=>new Date(b.created_at)-new Date(a.created_at))[0];return{client:key,registered:matches.length>0,connected:authorized.length>0,activeConnections:authorized.length,lastConnectedAt:latest?.created_at||null}});
 return send(res,200,{status:"ok",server:"Bytly MCP",clients:result,registeredClients:(clients||[]).length,activeTokens:(tokens||[]).filter(t=>new Date(t.refresh_expires_at||t.expires_at||0).getTime()>Date.now()).length,checkedAt:new Date().toISOString()});
}
