...
// Also called internally by platformAgent (the unified assistant) via
// base44.functions.invoke('platformDataAssistant', ...) — this is the
// single implementation; platformAgent does not duplicate this registry.
//
...
4. detected_intent: one short sentence (in English, for internal logging) describing what the admin is actually asking for.`,
    ...
    if (routing.capability === 'none' || routing.coverage === 'unsupported') {
      ...
      return Response.json({
        success: true, answer: cannotAnswer, language, coverage: 'unsupported',
...
${JSON.stringify({ stats, sample_rows: rows.slice(0, 20) })}`,
    ...
    return Response.json({
      success: true, answer: composed.answer, language, coverage,
