// ════════════════════════════════════════════════════════════════════════
// Shared, read-only data-question capability registry + runner.
// Used by BOTH platformDataAssistant/entry.ts (standalone endpoint, kept
// for backwards compatibility) and platformAgent/entry.ts (the unified
// agent). Single source of truth — do not fork this logic between them.
//
// Hard rule: every run() below may only call .filter()/.list(). None may
// call .create()/.update()/.delete() on platform data.
// ════════════════════════════════════════════════════════════════════════
...
// Full routing + execution + compose + audit-log pipeline, shared by both
// the standalone platformDataAssistant endpoint and the unified agent.
export async function answerDataQuestion(base44, user, question) {
...
4. detected_intent: one short sentence (in English, for internal logging) describing what the admin is actually asking for.`,
  ...
  if (routing.capability === 'none' || routing.coverage === 'unsupported') {
    ...
    return {
      kind: 'data', success: true, answer: cannotAnswer, language, coverage: 'unsupported',
      capability_used: null, admin_page: null, read_only: true, table: null, stats: {},
    };
  }
...
${JSON.stringify({ stats, sample_rows: rows.slice(0, 20) })}`,
  ...
  return {
    kind: 'data', success: true, answer: composed.answer, language, coverage,
    capability_used: capability.key, admin_page: capability.admin_page, read_only: true,
