...
const adminClient = createClient(supabaseUrl, secretKeys.default || Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!);
...
async function requireAdmin(req: Request) {
  ...
  if ((profile?.role || data.user.app_metadata?.role) !== "admin") throw new Error("FORBIDDEN");
  return data.user;
}
...
async function getConnection(userId: string) {
  const { data, error } = await adminClient
    ...
    .select("id,user_id,provider,provider_email,refresh_token,access_token,expires_at,status,scopes")
    .eq("user_id", userId)
    .eq("provider", "gmail")
    .maybeSingle();
...
async function saveConnection(userId: string, patch: Record<string, unknown>) {
  ...
  const { error } = await adminClient.from("integration_connections").insert({
    user_id: userId, provider: "gmail", status: "connected", ...patch,
  });
  if (error) throw error;
}
...
async function refreshGoogleToken(refreshToken: string) {
  ...
  if (!clientId || !clientSecret) throw new Error("Gmail OAuth credentials are not configured in Supabase Edge Function Secrets.");
...
async function getValidAccessToken(userId: string, forceRefresh = false) {
  ...
  if (!forceRefresh && connection.access_token && expiresAt > Date.now() + 60_000) {
    return connection.access_token;
  }

  if (!connection.refresh_token) throw new Error("Gmail refresh token is missing. Please re-authorize Gmail.");
...
function buildRawMessage(to: string, subject: string, text: string, html?: string, from?: string) {
  ...
  return base64UrlEncode([
    ...headers, "", "--bytly-boundary",
    "Content-Type: text/plain; charset=UTF-8", "Content-Transfer-Encoding: 8bit", "", text,
    "--bytly-boundary", "Content-Type: text/html; charset=UTF-8",
    "Content-Transfer-Encoding: 8bit", "", safeHtml, "--bytly-boundary--",
  ].join("\r\n"));
}

function parseEmailBody(payload: any): string {
  if (!payload) return "";
  if (payload.body?.data) { try { return decodeBase64Utf8(payload.body.data); } catch {} }
  if (payload.parts) {
    for (const mimeType of ["text/html", "text/plain"]) {
      for (const part of payload.parts) {
        if (part.mimeType === mimeType && part.body?.data) {
          try { return decodeBase64Utf8(part.body.data); } catch {}
        }
      }
    }
    for (const part of payload.parts) {
  ...
  try {
    ...
    if (action === "storeProviderTokens") {
      ...
      if (!providerToken || !providerRefreshToken) {
        return jsonResponse({ ok: false, needs_reauth: true, error: "Google did not return a refresh token. Re-authorize Gmail with offline access." }, 400);
      }
      ...
      if (!profileResponse.ok) return jsonResponse({ ok: false, error: profile?.error?.message || "Invalid Gmail token." }, 401);
      ...
      await saveConnection(user.id, {
        ...
        status: "connected",
        scopes: ["openid","email","profile","https://www.googleapis.com/auth/gmail.send","https://www.googleapis.com/auth/gmail.readonly","https://www.googleapis.com/auth/gmail.settings.basic"],
        last_checked_at: new Date().toISOString(),
        last_error: null,
      });
      return jsonResponse({ ok: true, email: profile?.emailAddress || null });
    }

    if (action === "status") {
      const profile = await gmailRequest(user.id, "/profile");
      await saveConnection(user.id, { provider_email: profile?.emailAddress || null, status: "connected", last_checked_at: new Date().toISOString(), last_error: null });
    ...
    if (action === "listEmails") {
      ...
      const emails = await Promise.all((list.messages || []).map(async (msg: any) => {
        try {
          ...
          return { id: detail.id, threadId: detail.threadId, snippet: detail.snippet, from: get("From"), to: get("To"), subject: get("Subject"), date: get("Date"), labelIds: detail.labelIds || [], isUnread: (detail.labelIds || []).includes("UNREAD") };
        } catch { return null; }
      }));
    ...
    if (action === "getEmail") {
      ...
      const headers = detail.payload?.headers || [];
      const get = (name: string) => headers.find((h: any) => h.name === name)?.value || "";
      return jsonResponse({ success: true, email: { id: detail.id, threadId: detail.threadId, from: get("From"), to: get("To"), subject: get("Subject"), date: get("Date"), body: parseEmailBody(detail.payload), labelIds: detail.labelIds || [], isUnread: (detail.labelIds || []).includes("UNREAD") } });
    }

    if (action === "sendEmail") {
      if (!data?.to || !data?.subject || !data?.body) return jsonResponse({ success: false, error: "to, subject and body are required." }, 400);
