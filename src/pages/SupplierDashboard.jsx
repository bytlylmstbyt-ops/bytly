...
import {
  ...
  Settings, Star, Clock, FileText, PackageSearch
} from "lucide-react";
...
export default function SupplierDashboard() {
      ...
      <div className="min-h-screen flex items-center justify-center">
        <Card className="max-w-md">
          <CardContent className="pt-6 text-center">
            <p className="text-slate-600 mb-4">لم يتم العثور على حساب مورد مرتبط بحسابك</p>
            <Link to={createPageUrl("RegisterSupplier")}>
              <Button>إنشاء حساب مورد</Button>
            </Link>
          </CardContent>
        </Card>
      </div>
  ...
  const statCards = [
    ...
    {
      title: "رصيد المحفظة",
      value: `${(supplier.wallet_balance || 0).toLocaleString('ar-SA')} ر.س`,
      icon: DollarSign,
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <Card className="border-0 shadow-lg overflow-hidden mb-8">
            <div
              ...
              style={{
                backgroundImage: supplier.cover_image ? `url(${supplier.cover_image})` : undefined,
              ...
              <div className="flex flex-col md:flex-row items-start md:items-end gap-4 -mt-16">
                <div className="w-32 h-32 rounded-2xl border-4 border-white shadow-lg bg-white flex items-center justify-center overflow-hidden">
                  {supplier.profile_image ? (
                    <img src={supplier.profile_image} alt={supplier.company_name} className="w-full h-full object-cover" />
                  ) : (
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-3 mb-2">
                    <h1 className="text-2xl font-bold text-slate-900">{supplier.company_name}</h1>
                    ...
                    <Badge className={
                      ...
                      {isApproved ? "معتمد" : supplier.status === "rejected" ? "مرفوض" : "قيد المراجعة"}
                    </Badge>
                  </div>
                  {supplier.bio && (
                    <p className="text-slate-600 mb-3">{supplier.bio}</p>
                  )}
                  <div className="flex flex-wrap gap-3 text-sm text-slate-600">
                    <span>📍 {supplier.city}</span>
                    <span>📦 {supplier.specialization}</span>
                    {supplier.years_experience > 0 && (
                      <span>📅 {supplier.years_experience} سنة خبرة</span>
                    )}
                    {supplier.website && (
          ...
          <Tabs defaultValue="available" className="space-y-6">
                ...
                <CardContent>
                  {availableProjects.length > 0 ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-slate-600">لا توجد طلبات متاحة حالياً</p>
                    </div>
                  )}
                </CardContent>
              </Card>
            ...
            <TabsContent value="mine">
              <Card className="border-0 shadow-lg">
                <CardHeader>
                  <CardTitle className="flex items-center justify-between">
                    <span>طلباتي ({projects.length})</span>
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  {projects.length > 0 ? (
                    ...
                    <div className="text-center py-12">
                      ...
                      <p className="text-slate-600 mb-3">لا توجد طلبات مسندة إليك بعد</p>
                      <p className="text-sm text-slate-400">تصفّح الطلبات المتاحة وقدم عروضك</p>
                    </div>
                  )}
                </CardContent>
              </Card>
