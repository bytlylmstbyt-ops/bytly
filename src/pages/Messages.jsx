...
function normalize(row, type) {
  ...
  return { id: row.id, email: String(row.email).trim().toLowerCase(), name: displayName(row), company_name: row.company_name || "", city: row.city || "", image: row.profile_image || row.company_logo || "", type, verified: !!row.is_verified };
}
...
export default function Messages() {
  ...
  const loadConversations = async () => {
    ...
    try {
      const result = await timeout(supabase.from("user_conversations").select("*").contains("participants", [me]).order("last_message_date", { ascending: false, nullsFirst: false }).order("created_at", { ascending: false }));
      if (result.error) throw result.error;
      setConversations(result.data || []);
    } catch (e) { console.error(e); setError(e?.message || "تعذر تحميل المحادثات"); }
  ...
  const loadDirectory = async () => {
    ...
    try {
      ...
      const map = new Map();
      const add = (rows, role) => (rows || []).forEach(row => { const p = normalize(row, role); if (p && p.email !== me) map.set(p.email, p); });
  ...
  const openConversation = async conversation => {
    ...
    try {
      const result = await timeout(supabase.from("user_messages").select("*").eq("conversation_id", conversation.id).order("created_at", { ascending: true }));
      if (result.error) throw result.error;
      setMessages(result.data || []);
    } catch (e) {
      console.error(e);
      toast.error(e?.message || "تعذر تحميل تفاصيل المحادثة");
    } finally { setLoadingMessages(false); }
  };

  useEffect(() => {
    if (isAuthenticated && me) { loadConversations(); loadDirectory(); }
    else { setLoading(false); setError("يجب تسجيل الدخول للوصول إلى المحادثات."); }
  }, [isAuthenticated, me]);
  ...
  const rows = useMemo(() => conversations.map(c => {
    ...
    const others = participants.filter(email => String(email).toLowerCase() !== me);
    const people = others.map(email => peopleByEmail.get(String(email).toLowerCase())).filter(Boolean);
    return { ...c, otherEmails: others, people, displayName: c.name || people.map(p => p.name).join("، ") || others.join("، ") || "محادثة مباشرة" };
  ...
  const startDirect = async person => {
    if (!person || !me) return;
    const existing = conversations.find(c => c.type === "direct" && Array.isArray(c.participants) && c.participants.length === 2 && c.participants.map(String).map(x => x.toLowerCase()).includes(me) && c.participants.map(String).map(x => x.toLowerCase()).includes(person.email));
    if (existing) { setNewChat(false); return openConversation(existing); }
    try {
      const now = new Date().toISOString();
      const payload = { id: crypto.randomUUID(), created_at: now, updated_at: now, project_id: "direct", type: "direct", name: person.name, participants: [me, person.email], participant_roles: { [person.type]: person.email }, admin_emails: [], is_archived: false, muted_by: [], is_main_room: false };
      const result = await timeout(supabase.from("user_conversations").insert(payload).select("*").single());
      if (result.error) throw result.error;
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const result = await timeout(supabase.from("user_messages").insert(payload).select("*").single());
      if (result.error) throw result.error;
  ...
  const call = video => {
    if (!selected) return;
    const other = (selected.participants || []).find(email => String(email).toLowerCase() !== me) || "user";
    const room = `Bytly-${selected.id}`;
    window.open(`https://meet.jit.si/${encodeURIComponent(room)}`, "_blank", "noopener,noreferrer");
    toast.success(video ? "تم فتح مكالمة الفيديو" : "تم فتح المكالمة");
  };
  ...
  return <div className="min-h-[70vh] bg-slate-50 p-3 md:p-6" dir="rtl">
    <div className="mx-auto max-w-7xl rounded-2xl border bg-white shadow-sm overflow-hidden">
      ...
      <div className="grid min-h-[650px] md:grid-cols-[330px_1fr]" dir="ltr">
        ...
        <main dir="rtl" className="flex min-h-[650px] flex-col">
          ...
          {selected&&<div className="border-t bg-white p-3"><div className="flex items-end gap-2"><label title="إضافة مرفق" className="cursor-pointer rounded-xl border p-3 hover:bg-slate-50"><Paperclip size={20}/><input type="file" className="hidden" onChange={attachFile}/></label><textarea value={draft} onChange={e=>setDraft(e.target.value)} onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();sendMessage();}}} rows={2} placeholder="اكتب رسالتك..." className="min-h-[48px] flex-1 resize-none rounded-xl border px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#C9A66B]"/><button onClick={sendMessage} disabled={!draft.trim()||sending} title="إرسال" className="rounded-xl bg-[#3b2418] p-3 text-white disabled:opacity-50">{sending?<Loader2 size={20} className="animate-spin"/>:<Send size={20}/>}</button></div><div className="mt-1 text-[11px] text-slate-400">Enter للإرسال · Shift+Enter لسطر جديد</div></div>}
        </main>
      </div>
    </div>
