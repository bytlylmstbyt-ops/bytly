...
const supabaseEntity = (table, keyMap = {}) => ({
  filter: async (filters = {}, sort = '-created_date', limit = 100) => {
    try {
      let q = supabase.from(table).select('*');
      for (const [key, value] of Object.entries(filters || {})) {
        const mapped = keyMap[key] || key;
        if (value === null) q = q.is(mapped, null);
        else if (Array.isArray(value)) q = q.in(mapped, value);
        else q = q.eq(mapped, value);
      }
      q = q.limit(limit);
      const orderColumn = sort.replace(/^-/, '');
      q = q.order(orderColumn === 'created_date' ? 'created_at' : orderColumn, { ascending: !sort.startsWith('-') });
...
legacyBase44.entities.Proposal = supabaseEntity('project_offers', { engineer_id: 'engineer_id', project_id: 'project_id' });
legacyBase44.entities.Review = supabaseEntity('project_reviews', { engineer_id: 'engineer_id', project_id: 'project_id' });

legacyBase44.entities.Notification = {
  ...supabaseEntity('notifications', { recipient_id: 'user_id', recipient_user_id: 'user_id' }),
  filter: async (filters = {}, sort = '-created_date', limit = 100) => {
    try {
      const userId = filters.recipient_id || filters.recipient_user_id;
      let q = supabase.from('notifications').select('*').limit(limit);
      if (userId) q = q.eq('user_id', userId);
      if (filters.is_read === true) q = q.not('read_at', 'is', null);
      if (filters.is_read === false) q = q.is('read_at', null);
      const { data, error } = await withHardTimeout(q.order('created_at', { ascending: false }), 10000, 'انتهت مهلة قراءة الإشعارات');
      if (!error) return (data || []).map(n => ({ ...n, is_read: Boolean(n.read_at), created_date: n.created_at }));
    } catch {}
    return [];
  }
};

legacyBase44.entities.Dispute = {
  ...supabaseEntity('disputes'),
  list: async (sort = '-created_date', limit = 100) => {
    try {
      const { data, error } = await withHardTimeout(supabase.from('disputes').select('*').limit(limit).order('created_at', { ascending: false }), 10000, 'انتهت مهلة قراءة النزاعات');
      if (!error) return data || [];
    } catch {}
    return [];
  }
...
legacyBase44.entities.ConsultationAppointment = { filter: async () => [], list: async () => [] };
