      ...
      <Dialog open={!!selectedProject} onOpenChange={(open) => !open && setSelectedProject(null)}>
        <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto" dir="rtl">
          ...
          {selectedProject && <div className="space-y-5">
            ...
            <Card><CardContent className="p-4"><h4 className="font-bold mb-3">مسار المشروع</h4>{selectedProject.phases?.length ? <div className="flex flex-wrap gap-2 items-center">{selectedProject.phases.map((phase,idx) => <React.Fragment key={phase.id || phase.key || idx}><Badge variant={phase.key === selectedProject.phase || phase.status === "in_progress" ? "default" : "outline"}>{phase.name || phase.label || phase.key}</Badge>{idx < selectedProject.phases.length - 1 && <span className="text-slate-300">←</span>}</React.Fragment>)}</div> : selectedProject.phase_history?.length ? <div className="flex flex-wrap gap-2 items-center">{selectedProject.phase_history.map((entry,idx) => <React.Fragment key={entry.id || `${entry.phase}-${idx}`}><Badge variant={entry.phase === selectedProject.phase ? "default" : "outline"}>{entry.label || entry.name || entry.phase}</Badge>{idx < selectedProject.phase_history.length - 1 && <span className="text-slate-300">←</span>}</React.Fragment>)}</div> : <div className="p-3 rounded-lg bg-slate-50 text-sm text-slate-500">لم يتم تحديد مسار مراحل لهذا المشروع بعد. لا يتم افتراض مراحل إضافية.</div>}{selectedProject.phase_history?.length > 0 && <p className="text-xs text-slate-500 mt-3">عدد انتقالات المرحلة: {selectedProject.phase_history.length}</p>}</CardContent></Card>
