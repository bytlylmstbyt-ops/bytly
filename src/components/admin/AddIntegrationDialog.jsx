          ...
          <div className="flex-1 overflow-y-auto">
            ...
            {selected.kind === "payment" ? (
              <div className="space-y-3 text-sm text-slate-600">
                <div className="rounded-md bg-emerald-50 border border-emerald-200 text-emerald-800 p-3 text-xs">Moyasar مضاف كبوابة الدفع داخل مركز الإدارة. الحساب الحالي يعمل في بيئة الاختبار إلى أن يكتمل التفعيل التجاري.</div>
                <p className="font-medium text-slate-700">من هنا نتابع:</p>
                <ul className="list-disc list-inside space-y-1.5">
                  <li>بيئة Test / Live</li>
                  <li>المفتاح العام فقط</li>
                  <li>حالة Webhook</li>
                  <li>فحص إعداد التكامل</li>
                </ul>
                <p className="text-xs text-slate-500">المفتاح السري محفوظ في Supabase Secrets ولا يظهر في لوحة الإدارة.</p>
                <Button variant="outline" className="w-full" onClick={handleRefresh} disabled={refreshing || !onRefresh}>
                  {refreshing ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />} تحديث الحالة
                </Button>
              </div>
