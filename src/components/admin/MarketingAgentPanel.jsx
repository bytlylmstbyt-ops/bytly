...
import { Brain, CheckCircle2, ClipboardCheck, Loader2, Sparkles } from 'lucide-react';
...
export default function MarketingAgentPanel() {
  ...
  const analyze = async () => {
    setLoading(true); setMessage(''); setRecommendations([]);
    try {
      const s = await getMarketingAgentSnapshot();
      setSnapshot(s);
      setInsights(buildMarketingInsights(s).sort((a, b) => (priorityRank[b.priority] || 0) - (priorityRank[a.priority] || 0)));
    } catch (error) {
      setMessage(error?.message || 'تعذر تحليل بيانات المنصة.');
  ...
  const saveDrafts = async () => {
    ...
    try {
      ...
      setMessage('تم حفظ الاقتراحات بحالة «مقترح من الوكيل». لم يتم تنفيذ أي مهمة.');
    } catch (error) {
      setMessage(error?.message || 'تعذر حفظ اقتراحات الوكيل.');
  ...
  const approveAll = async () => {
    ...
    try {
      ...
      setMessage('تم اعتماد الاقتراحات. لم يتم نشر محتوى أو إنفاق ميزانية.');
  ...
  return <div className="space-y-6" dir="rtl">
    <Card>
      ...
      <CardContent className="space-y-4">
        <p className="text-sm text-muted-foreground">يحلل بيانات بيتلي الفعلية ويقترح قنوات ومبادرات تسويقية. الأولويات تتغير حسب البيانات المتاحة، ولا يوجد نشر أو إنفاق أو تنفيذ تلقائي.</p>
