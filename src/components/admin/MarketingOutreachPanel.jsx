...
import { Mail, MessageCircle, CheckCircle2, Copy, Eye, RefreshCw, Users, Clock } from 'lucide-react';
...
export default function MarketingOutreachPanel() {
  ...
  const prepareMessage = async (contact) => {
    ...
    try {
      ...
      setNotice('تم تجهيز الرسالة كمسودة مقترحة. لم يتم إرسالها تلقائيًا.');
  ...
  const confirmSent = async (contact) => {
    ...
    try {
      ...
      setNotice('تم تسجيل التواصل. هذا يعني أنك أكدتِ أن الرسالة أُرسلت فعليًا.');
    } catch (e) { setNotice(`تعذر تسجيل التواصل: ${e?.message || 'خطأ'}`); }
    finally { setBusy(null); }
  };
  ...
  return <div dir="rtl" className="space-y-5">
    <Card className="border-[#C9A66B]/30">
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Button variant="outline" size="sm" onClick={load}><RefreshCw className="w-4 h-4 ml-2" />تحديث</Button>
      </CardHeader>
      <CardContent>
        <p className="text-sm text-slate-500 mb-4">الوكيل يعرض العملاء والمهندسين والشركات الحقيقية الموجودة في المنصة، مع سجل واضح لما تم إرساله فعليًا.</p>
        ...
        {loading ? <div className="py-8 text-center text-slate-500">جاري تحميل جهات التواصل...</div> : contacts.length === 0 ? <div className="py-8 text-center text-slate-500">لا توجد جهات حقيقية متاحة للتواصل حاليًا.</div> : <div className="space-y-3">
    ...
    {selected && <Card>
      <CardHeader className="flex flex-row items-center justify-between"><CardTitle>رسائل {selected.name}</CardTitle><Button variant="ghost" onClick={() => setSelected(null)}>إغلاق</Button></CardHeader>
