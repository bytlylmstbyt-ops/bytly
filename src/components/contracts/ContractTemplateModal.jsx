...
/**
 * ContractTemplateModal — Bytly-branded, draggable, animated contract creator.
...
المادة (7) — السريّة وأحكام عامة:
  ...
  يخضع للأنظمة المعمول بها في المملكة العربية السعودية.
    ...
    <AnimatePresence>
      ...
      >
        ...
        <motion.div
          ...
          whileDrag={{ boxShadow: "0 30px 80px rgba(0,0,0,0.35)" }}
          className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[92vh] overflow-hidden flex flex-col"
          onClick={(e) => e.stopPropagation()}
        >
          {/* ── Branded Header (drag handle) ─────────────────────── */}
          <div
            onPointerDown={(e) => dragControls.start(e)}
            className="bg-gradient-to-l from-[#4A3F35] via-[#6B5D4F] to-[#C9A66B] px-5 sm:px-7 py-4 flex items-center justify-between shrink-0 cursor-grab active:cursor-grabbing touch-none"
          >
            <div className="flex items-center gap-3 pointer-events-none select-none">
              ...
              <div>
                ...
                <p className="text-white/70 text-xs">اسحب النافذة من هنا</p>
              </div>
            </div>
            <button onClick={onClose} className="text-white/70 hover:text-white p-1.5 rounded-lg hover:bg-white/10 shrink-0">
              <X className="w-5 h-5" />
            </button>
          </div>
