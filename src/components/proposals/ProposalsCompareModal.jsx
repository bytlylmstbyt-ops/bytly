...
import { Star, Clock, DollarSign, CheckCircle, Award, Loader2, ArrowUp, ArrowDown, Search, TrendingDown, Zap } from "lucide-react";
...
export default function ProposalsCompareModal({ open, onOpenChange, proposals, engineers, onAccept, acceptingId }) {
  ...
  const rows = useMemo(() => {
    if (!proposals || proposals.length < 2) return [];
    ...
    list = [...list].sort((a, b) => (sortDir === "asc" ? val(a) - val(b) : val(b) - val(a)));
    return list;
  }, [proposals, engineers, search, statusFilter, sortKey, sortDir]);

  const stats = useMemo(() => {
    if (!proposals || proposals.length === 0) return { minPrice: 0, minDays: 0, maxRating: 0 };
    return {
  ...
  const SortHeader = ({ label, icon: Icon, k, hint }) => (
      ...
      <DialogContent className="max-w-5xl max-h-[90vh] overflow-y-auto" dir="rtl">
        <DialogHeader>
          <DialogTitle className="text-right flex items-center gap-2">
            ...
            مقارنة العروض — جدول تفاعلي
          </DialogTitle>
          <DialogDescription className="text-right">
            قارن بين {proposals.length} عروض حسب السعر ومدة التنفيذ والتقييم. اضغط على رأس العمود للترتيب، واستخدم البحث والفلتر للفرز.
          </DialogDescription>
        </DialogHeader>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th className="px-3 py-2.5 text-right text-xs font-semibold text-slate-600">رسالة العرض</th>
                  ...
                  <tr key={p.id} className="border-t border-slate-100 hover:bg-amber-50/40 align-top">
                    ...
                    <td className="px-3 py-3">
                      {p.status !== "accepted" && onAccept ? (
                        <Button size="sm" className="bg-gradient-to-r from-[#6B5D4F] to-[#C9A66B] text-white" disabled={acceptingId === p.id} onClick={() => { onAccept(p.id); onOpenChange?.(false); }}>
                          {acceptingId === p.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle className="w-4 h-4 ml-1" />}
                          اختيار
                        </Button>
                      ) : (
                        <Badge className="bg-green-100 text-green-700 text-[10px]">مختار</Badge>
                      )}
                    </td>
                  </tr>
                );
        ...
        <p className="text-xs text-slate-400 mt-2 flex items-center gap-1.5">
          ...
          القيم الأفضل مُميّزة تلقائياً (أقل سعر / أسرع تسليم / أعلى تقييم) لتسهيل اختيار الأنسب.
        </p>
      </DialogContent>
    </Dialog>
  );
