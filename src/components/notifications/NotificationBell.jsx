...
export default function NotificationBell() {
  ...
  return <DropdownMenu open={isOpen} onOpenChange={setIsOpen}>
    ...
    <DropdownMenuContent align="end" className="w-80 max-h-96 overflow-y-auto">
      ...
      {notifications.length===0?<div className="text-center py-8 text-slate-500 text-sm">لا توجد إشعارات</div>:<>{notifications.map(n=><DropdownMenuItem key={n.id} asChild><Link to={createPageUrl("NotificationCenter")} className={`flex items-start gap-3 p-3 cursor-pointer transition-colors ${!n.read_at?"bg-blue-50 hover:bg-blue-100":"hover:bg-slate-50"}`}>
      ...
      </Link></DropdownMenuItem>)}<DropdownMenuSeparator/><DropdownMenuItem asChild><Link to={createPageUrl("NotificationCenter")} className="text-center text-sm text-[#C9A66B] font-medium p-2 hover:bg-amber-50">عرض كل الإشعارات</Link></DropdownMenuItem></>}
    </DropdownMenuContent>
  </DropdownMenu>;
}